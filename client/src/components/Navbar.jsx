    ...
    <nav className="bg-white shadow-sm sticky top-0 z-30">
      <div className="mx-auto flex h-14 items-center justify-between px-4 sm:px-6 lg:max-w-6xl">
        ...
        <div className="hidden md:flex items-center gap-4 text-sm">
          ...
          <Link to="/posts" className="hover:text-gray-900 text-gray-700">Posts</Link>
          <Link to="/channels" className="hover:text-gray-900 text-gray-700">Channels</Link>
          {user && <Link to="/chat" className="hover:text-gray-900 text-gray-700">Chat</Link>}
          {user?.role === 'employer' && (
            <Link to="/interested" className="hover:text-gray-900 text-gray-700">Interested</Link>
          )}
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-200 bg-white shadow-lg">
          <div className="px-4 py-4 flex flex-col gap-3 text-sm">
            ...
            <Link to="/posts" onClick={() => setMobileOpen(false)} className="py-1">Posts</Link>
            <Link to="/channels" onClick={() => setMobileOpen(false)} className="py-1">Channels</Link>
            {user && <Link to="/chat" onClick={() => setMobileOpen(false)} className="py-1">Chat</Link>}
            {user?.role === 'employer' && (
              <Link to="/interested" onClick={() => setMobileOpen(false)} className="py-1">Interested</Link>
            )}
