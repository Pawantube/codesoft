  ...
  const renderAuthLinks = (className = '') => {
    ...
    return (
      <div className={`flex items-center gap-3 ${className}`.trim()}>
        <Link to="/login" className="text-sm" onClick={() => setMobileOpen(false)}>
          Login
        </Link>
        <Link
          to="/register"
  ...
  return (
    <nav className="bg-white shadow-sm sticky top-0 z-30">
      <div className="mx-auto flex h-14 items-center justify-between px-4 sm:px-6 lg:max-w-6xl">
        <Link to="/" className="font-bold text-lg text-gray-900" onClick={() => setMobileOpen(false)}>
          SawConnect
        </Link>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-200 bg-white shadow-lg">
          <div className="px-4 py-4 flex flex-col gap-3 text-sm">
            ...
            {user?.role === 'candidate' && (
              <Link to="/candidate" onClick={() => setMobileOpen(false)} className="py-1">
                Dashboard
              </Link>
            )}
            {user && (
              <Link to="/profile" onClick={() => setMobileOpen(false)} className="py-1">
                Profile
              </Link>
            )}
            {user && (
            ...
            {user?.role === 'employer' && (
              <Link
                to="/employer/post"
                onClick={() => setMobileOpen(false)}
                className="px-3 py-2 rounded-lg bg-gray-900 text-white text-center"
              >
                Post a Job
              </Link>
            )}
