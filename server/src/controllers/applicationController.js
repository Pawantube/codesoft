...
export const updateStatus = async (req, res) => {
  ...
  await app.save();
  try {
    await notify({ user: app.candidate._id, title: status==='accepted'?'Application accepted': status==='rejected'?'Application rejected':'Application update', message:`${app.job.title} at ${app.job.company} — status: ${status}`, link:`/jobs/${app.job._id}`, type:'status', req });
